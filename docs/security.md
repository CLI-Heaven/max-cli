...
| Куда | Когда |
|---|---|
| `wss://ws-api.oneme.ru/websocket`, заголовок `Origin` — `https://web.max.ru` | каждая команда, которой нужен MAX |
...
взяты у веб-клиента MAX. Остальное отличается: веб-клиент шлёт двоичные кадры и свою телеметрию, а
`max` — текстовые кадры JSON и телеметрии не шлёт. Поэтому MAX может отличить `max` от веб-клиента.
