...
| модели распознавания речи — **только после `max models download`** | `~/.cache/max-cli/models/…` | каталог `0700`, файлы `0600` |
...
| Hugging Face и GitHub — файлы модели распознавания речи | только `max models download`; голос туда не уходит — он распознаётся на этом компьютере |
