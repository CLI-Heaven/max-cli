...
```sh
max session start qr-chrome  # вход через web.max.ru, токен уйдёт в ключницу
...
Проще всего — `max session start qr-chrome`: откроется web.max.ru, вы отсканируете QR-код
приложением MAX, и токен ляжет в ключницу. Все пять способов входа —
