...
Все сообщения — и ваши, и чужие — во всех чатах, где что-то было с `--since`. Нужно, чтобы понять,
...
начинайте с этого `--since`, тогда ничего не потеряется между двумя обзорами. Если обзор неполный —
...
`--notify` тоже считаются; тихое закрепление проходит проверки, но в лимит не идёт. Если пересылка ответила `outcome_unknown`, повторите
её с `--cid` из ошибки.
...
его видно через `contacts show <id>`. Название папки — не длиннее 20 знаков: длиннее MAX не
принимает, и `max` откажет сам, ничего не отправляя. `import` отправляет в MAX номера других людей.
...
журнал отправок попадает действие без названий и ссылок. `create` и `members add` считаются в лимит
отправок в час по одному на каждого человека — им приходит сообщение. Если список получателей
включён, людей можно звать, только если личный чат с каждым есть в списке. Ничего не повторяется при сбое: повтор `create` — вторая группа.
...
**Сервер, запущенный руками (`max serve`, `max server start`), останавливают только Ctrl-C и
`max server stop`**: ни простой, ни `max session end`, ни другие запросы по сокету. Если в фоне
...
- **После обновления `max`** сервер, который запустила команда, сменяется новым сам. Запущенный
  руками работает на старой версии, пока его не перезапустить: `max server status` это покажет,
  `max server restart` исправит.
...
По умолчанию записывается только запуск, который кончился ошибкой ([diagnostics.md](diagnostics.md)).
...
`max backup messages` дочитывает историю одного чата в локальную копию — назад до даты или до
числа сообщений:

```sh
max backup messages Друзья --since 2026-01-01
max backup messages Друзья --since 2026-01-01 --run
```

Без `--run` команда **только считает** и в MAX не ходит: сколько сообщений уже есть, каких кусков
не хватает, сколько это страниц запросов, запусков и минут. Для `--last <n>` счёт точный, для
`--since` — оценка по тому, как часто в чате писали в уже прочитанных кусках.

С `--run` она листает назад так же, как веб-версия при прокрутке вверх: по 30 сообщений, от самого
старого загруженного. Между страницами — пауза от `--pause` до вдвое большей (по умолчанию 5–10 с: примерно так страницы идут, когда человек листает чат во вкладке).
За один запуск — не больше `--max-pages` страниц (по умолчанию 40, это около 1200 сообщений). Та же
команда ещё раз продолжает с места, где остановилась, и пропускает то, что уже прочитано.

На первой же ошибке MAX команда останавливается и не повторяет запрос. Прочитанное до ошибки
остаётся в копии. Реакции бэкап не читает, прочитанным ничего не отмечает.

Бэкап — не файл: сообщения ложатся в локальную копию профиля (путь показывает `max doctor`). Файл
делает `max export messages` — команда с `--run` в конце печатает её готовой, например
`max export messages 0 --format md --output chat-0.md`.
