...
quiz}`. Опрос новой версии, которую не показывает и веб-клиент, печатается одной строкой без
вариантов.
...
же проверки, что отправка, и **считаются в `sendsPerHour`** — чтобы не упереться в ограничения MAX.
Голос не повторяется сам. Для агентов: `max_polls_vote` и `max_polls_create` в `max mcp` — только с
`--allow-send`.
