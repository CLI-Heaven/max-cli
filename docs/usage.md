...
подключение к MAX открытым, пока его не остановят, и раздаёт новые сообщения тем, кто слушает.

```sh
max serve                   # в одном терминале; Ctrl-C — остановить
...
- **Работает на переднем плане.** Держать его в фоне — дело systemd или `nohup`.
