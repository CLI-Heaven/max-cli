...
**Сервер, запущенный руками (`max serve`), ничто не останавливает, кроме Ctrl-C**: ни простой, ни
`max session end`, ни запрос по сокету. Если в фоне уже работает сервер, который запустила
