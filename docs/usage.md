...
`max serve --stop`**: ни простой, ни `max session end`, ни другие запросы по сокету. Если в фоне уже работает сервер, который запустила
команда, `max serve` руками сменяет его. `--idle` руками — пожалуйста: это ваш собственный выбор.
...
- **Пока сервер запущен, остальные команды не входят в MAX заново**: вход им отдаёт сервер, а
  чтение — история, контакты, файлы, реакции — идёт через его подключение. **Отправка и реакции
  идут своим подключением**, с обычным входом, через те же защиты. Сервера нет или он
  не отвечает — команда работает как раньше, сама.
...
- Сервер держит свой вход в актуальном состоянии: новое сообщение, чат прочитан на телефоне,
  ...
  сообщения), команды на время снова входят сами, а сервер в фоне входит заново — не чаще раза в
  минуту.
