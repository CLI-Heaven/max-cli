...
Что пришло во все чаты с прошлой проверки — одной командой:

```sh
max inbox                           # чужие сообщения с прошлого раза, по времени, у каждого — чат
max inbox --json                    # { since, until, chats: [...], skipped: [...] }
max inbox --since 2026-09-24T09:00  # разовый взгляд с этого времени
```

**Место, где остановились, хранится в профиле** и сдвигается, только когда вывод напечатан. Поэтому
`max inbox` по расписанию показывает каждое сообщение один раз. `--since` его не двигает. Первый
запуск смотрит на последние 24 часа. Свои сообщения не показываются. Больше `--limit` сообщений в
одном чате (по умолчанию 20) — показаны самые новые, а в stderr — команда, которой дочитать
остальное. За один запуск читается не больше 20 чатов; остальные изменившиеся названы в stderr и в
`skipped`.
...
Реакцию видит собеседник, как и ответ. Снять реакцию пока нельзя.
