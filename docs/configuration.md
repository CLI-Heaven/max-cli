...
`first word`, `MAX_PROFILE`, `config file` или `default`. `configFound: false` значит, что файла
...
| `serve` | запускать ли `max serve` в фоне, когда команде нужен MAX, а сервера нет; `--no-serve` — на один запуск | `true` |
