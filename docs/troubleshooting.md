...
Печатает то, от чего зависит любая команда, **не подключаясь к MAX** (если не указан `--online`): есть ли токен и откуда,
...
Один вход, один чат из списка, затем сервер MCP запускается так, как его запускает клиент, и
...
Первая команда называет каталог, вторая — что сейчас в `PATH`. Если каталог в списке есть, а `max`
...
**Linux и macOS.** Каталог команд npm — `$(npm prefix -g)/bin`. `max doctor` напечатает строку
