...
Если симптома здесь нет, начните с `--verbose`: он показывает, дошёл ли запрос до MAX и что
...
```sh
max chats list --verbose     # видны ли запросы к MAX
...
- вывод команды с `--verbose` — в нём нет ни имён, ни текстов, ни токена;
