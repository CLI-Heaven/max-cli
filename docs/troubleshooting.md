...
**Просто подождите.** Вход раньше срока — ещё одна попытка, которую MAX засчитает, а повторные
...
`create` пишет файл `max-report-<время>.json` в текущий каталог (права `0600`). В нём версия, среда
...
нём нет. Запуск, кончившийся ошибкой, сохраняется сам, даже без `--record`
([diagnostics.md](diagnostics.md)). Про другой запуск — `--run <id>`, номера показывает
`max runs list`.

Дальше команда печатает ссылку: она откроет в почтовой программе письмо на `reports@neirox.ai` с
темой и заготовкой текста. Файл приложите к письму сами и напишите, что делали и что случилось.
Если почтовой программы нет, напишите на этот адрес с любой почты.

Можно и публично, в [github.com/leemour/max-cli/issues](https://github.com/leemour/max-cli/issues).
