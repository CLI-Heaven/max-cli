...
Одно событие на запрос, два места, куда его можно направить. `--verbose` показывает и ничего не
хранит, `--record` хранит и ничего не показывает. По умолчанию не происходит ни того, ни другого.
...
```sh
max chats list --json --verbose > chats.json    # данные в файл, диагностика на экран
```
...
```sh
max chats list --json --verbose 2>&1 >/dev/null | jq -c 'select(.event == "response")'
```

`--verbose` сильнее `--quiet`: флаг, который человек дописал руками, обязан сработать
...
| код ошибки | текст ошибки, пришедший от MAX |
...
Текст ошибки MAX не записывается по той же причине: отказ сервера цитирует то, что мы послали, а
послали мы в том числе сообщение.
