...
`max` входит в ваш аккаунт так же, как веб-клиент [web.max.ru](https://web.max.ru): по тому же
WebSocket-протоколу и с токеном вашей сессии. Бот не нужен, всё делается от вашего имени. Каждый
...
- **Искать.** По тексту прочитанных сообщений и по названиям чатов — в локальной копии, без
  подключения к MAX.
